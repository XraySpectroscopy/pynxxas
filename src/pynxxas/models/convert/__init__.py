...
def convert_model(
    ...
    mod_to = _CONVERT_MODULE.get(type(instance))
    mod_from = _CONVERT_MODULE.get(model_type)
    if mod_to is None or mod_from is None:
        raise NotImplementedError(
            f"Conversion from {type(instance).__name__} to {model_type.__name__} is not implemented"
        )

    for nxxas_model in mod_to.to_nxxas(instance):
        yield from mod_from.from_nxxas(nxxas_model)
